...
//基于定位误差椭圆的参数（长轴、短轴和偏航角），创建一个可视化的标记（Marker），以在RViz中显示误差椭圆。
...
//处理Odometry消息的回调函数，计算误差椭圆的参数，发布误差椭圆的可视化标记，并评估定位精度。
