...
//    当use_odom_为真时，处理从订阅的里程计主题接收到的消息。
//    调用estimateAccel函数来估算加速度。
...
//    当use_odom_为假时，处理从订阅的TwistWithCovariance主题接收到的消息。
//    同样调用estimateAccel函数来估算加速度。
...
//    使用当前和前一个Twist消息来估算线性和角加速度。
//    应用低通滤波器（LowpassFilter1d）平滑加速度数据。
//    发布计算得到的加速度数据。
