...
//转换协方差矩阵，通过取三个方向（X, Y, Z）协方差的最大值来简化处理。
...
//结合车辆的里程计（即速度）信息和陀螺仪（IMU）数据来估计车辆的运动状态
...
//处理接收到的车辆速度消息，将其添加到队列中，并尝试结合陀螺仪数据。
...
//处理接收到的IMU（陀螺仪）消息，将其转换到基座坐标系中，并添加到队列中，然后尝试与车辆速度数据结合。
...
//发布处理后的数据，包括原始速度、速度与协方差、以及经过陀螺仪偏差校正后的速度和速度与协方差。
