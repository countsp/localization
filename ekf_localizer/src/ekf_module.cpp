...
: warning_(std::move(warning)),
  ...
  accumulated_delay_times_(params.extend_state_step, 1.0E15), //延迟时间被假定为非常大，表示在开始时没有有效的或可靠的延迟估计
  params_(params)
{
  Eigen::MatrixXd X = Eigen::MatrixXd::Zero(dim_x_, 1);
  
  //以一个非常大的数初始化，表示初始状态的高不确定性。
  ...
  kalman_filter_.init(X, P, params_.extend_state_step);//(x:[6*1] P:[6*6])
}

/*-------------------------------------------------------------------*/


//初始化扩展卡尔曼滤波器（EKF）的状态向量和协方差矩阵
...
//获取当前经过扩展卡尔曼滤波器（EKF）处理的位姿估计
//包括位置（x, y, z）和方向（偏航角、横滚角、俯仰角）
  ...
  bool get_biased_yaw) const
{
  ...
  */
  ...
  //构建位姿消息 current_ekf_pose
  Pose current_ekf_pose; //using Pose = geometry_msgs::msg::PoseStamped
  ...
  if (get_biased_yaw) {
    current_ekf_pose.pose.orientation =
      tier4_autoware_utils::createQuaternionFromRPY(roll, pitch, biased_yaw);//rpy to quaternion
  } else {
    current_ekf_pose.pose.orientation =
...
//从扩展卡尔曼滤波器（EKF）中获取当前的运动学状态（即速度和角速度）
...
/*-------------------------------------------------------------------*/
// uses: covariance.cpp::ekfCovarianceToPoseMessageCovariance
// EKF 的状态协方差矩阵转换成一个 36 元素的数组，这个数组代表 ROS 中 PoseWithCovariance 消息的协方差部分。
...
// uses: covariance.cpp::ekfCovarianceToTwistMessageCovariance
...
//在一个延迟时间数组 accumulated_delay_times_ 中找到最接近给定目标值 target_value 的元素的索引
...
/*--------------------!!!!!!!!!! Predict !!!!!!!!!------------------------*/
// X_curr 和 P_curr 分别是当前的状态向量和协方差矩阵
// uses: state_transition.cpp::predictNextState()
// uses: state_transition.cpp::createStateTransitionMatrix()
// uses: state_transition.cpp::processNoiseCovariance    ()
...
/*--------------------------------------------------------------------------*/
//将位姿（位置和方向）测量数据更新到扩展卡尔曼滤波器（EKF）中。
//该方法处理时间延迟，并确保测量数据与预定的坐标系一致。
...
{
  //验证传入的位姿测量数据 (pose) 的坐标系 (frame_id) 是否与预设的坐标系 (params_.pose_frame_id) 一致。
  ...
  //计算测量数据的时间戳 (pose.header.stamp) 与当前时间 (t_curr) 之间的差异，并加上额外的预设延迟 (params_.pose_additional_delay)。
  //如果计算出的延迟时间为负，则记录警告。
  ...
  //更新诊断信息 (pose_diag_info)，包括实际的延迟时间和延迟时间的阈值
  //检查延迟步数是否超过了预定的步数限制 (params_.extend_state_step)。如果超过，记录警告，并表明位姿测量更新失败,return。
  ...
  /* Since the kalman filter cannot handle the rotation angle directly,
    ...
    than 2 pi. */
  //为了使偏航角与 EKF 内部持有的偏航角误差小于 2π，对偏航角进行调整。
  ...
  //创建包含位置和调整后偏航角的测量矩阵 y
  Eigen::MatrixXd y(dim_y, 1);
  y << pose.pose.pose.position.x, pose.pose.pose.position.y, yaw;

  //测量数据有效性检查：
  //检查测量矩阵 y 是否包含 NaN 或 Inf。如果是，则忽略更新并返回 false。
  ...
  /* Gate */
  //使用马氏距离检查测量更新是否可接受。如果马氏距离超过设定阈值，则忽略测量更新。
  //uses:mahalanobis.cpp mahalanobis()
  ...
  //计算位姿测量矩阵 C 和测量噪声协方差矩阵 R。
  //uses: measurement.cpp poseMeasurementMatrix()
  //uses: measurement.cpp poseMeasurementCovariance()
...
//对给定的位姿（PoseWithCovariance）进行补偿，以考虑沿Z轴的延迟
  ...
  const PoseWithCovariance & pose, const double delay_time)
{
  ...
  const double dz_delay = kalman_filteer_.getXelement(IDX::VX) * dlay_time * std::sin(-rpy.y);
...
//在扩展卡尔曼滤波器（EKF）中进行速度（Twist）测量的更新。
//这个方法主要涉及处理和融合速度数据，并考虑时间延迟以及数据的一致性和准确性。
...
{
  //确保传入的速度测量数据 twist 的坐标系 frame_id 是 "base_link"。如果不是，则记录警告。
  ...
  /* Calculate delay step */
  //计算测量数据的时间戳与当前时间之间的差异，并加上预设的额外延迟 params_.twist_additional_delay。
  //如果延迟时间为负，则记录警告
  ...
  //根据计算出的延迟时间，使用 find_closest_delay_time_index 函数确定对应的延迟步数。
  const int delay_step = static_cast<int>(find_closest_delay_time_index(delay_time));
  
  //更新包括延迟时间和延迟时间阈值的诊断信息。如果延迟步数超过限制，则更新失败并返回 false。
  ...
  //计算速度测量矩阵 C 和测量噪声协方差矩阵 R。
  //uses: measurement.cpp twistMeasurementMatrix()
  //uses: measurement.cpp twistMeasurementCovariance()
