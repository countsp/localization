...
#include <iostream>
#include <numeric>
//通过考虑以往试验的结果来智能地指导超参数的搜索空间，从而在超参数优化过程中逐步缩小搜索范围，以期找到最优解。
...
//接收优化方向（最大化或最小化）、启动试验的数量、以及变量是否为循环变量的信息
...
//接收一个试验结果并添加到内部列表中，同时按照优化方向对试验结果进行排序。
...
//基于当前的试验结果和TPE算法的逻辑，生成下一个试验的超参数输入。
...
//计算给定输入相对于当前模型的对数似然比，用于评估该输入的优劣。
...
//根据试验的数量，生成权重，这些权重用于计算“好”的试验和“坏”的试验的概率密度估计。
...
//将循环变量标准化到[-1, 1)的范围内，以处理循环变量的周期性。
