...
//定义了一个名为SmartPoseBuffer的类，用于管理位姿数据的缓存，并提供时间戳插值的功能。

//初始化SmartPoseBuffer对象，设置日志记录器、位姿超时秒数和位姿距离容差。
...
//在给定的目标时间戳上插值位姿数据。
//返回：std::optional<InterpolateResult>，包含插值位姿和用于插值的两个原始位姿
